...
2.  **EXIF Extraction:** Before sending to the server, the client uses `ExifReader` to parse the image's raw bytes for EXIF, IPTC, and XMP metadata *(Camera make & model, Date taken, GPS coordinates, Software used e.g., Photoshop, DALL-E)*.
3.  **Edge Function Processing:** The base64 image and metadata summary are sent to the `detect-deepfake` edge function. This sends the image to **Gemini 2.5 Flash** with a specialized prompt and receives a structured JSON response (verdict, confidence, reasoning).
...
5.  **Result Display:** The client renders a color-coded verdict badge (green = real, red = fake), confidence progress bar, metadata analysis section, and AI reasoning text.
...
To prevent AI-generated images (which typically lack metadata) from receiving high authenticity scores, the following logic is strictly applied:
```javascript
